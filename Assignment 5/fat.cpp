// Operating Systems University of Calgary
...
{
	unordered_map<int, std::vector<int>> usedBlocks;
	for (int i = 0; i < files.size(); i++) {
		int blocksExpected = ceil((float)files[i].size / (float)blockSize);
		...
		while (block != -1) {
			...
			if (usedBlocks.count(block) > 0) {
				if (usedBlocks.at(block).size() == 1) {
					usedBlocks.at(block).push_back(i);
					int index = usedBlocks.at(block)[0];
					files[index].sharesBlocks = true;
					index = usedBlocks.at(block)[1];
					files[index].sharesBlocks = true;
				}
				else {
					usedBlocks.at(block).push_back(i);
				...
				std::vector<int> list;
				list.push_back(i);
				usedBlocks.insert({block, list});
			}
			block = fat[block];
		}
		int blocksActual = path.size();
